...
def build_vectorstore(text, api_key):
    ...
    try:
        ...
        if 'insufficient_quota' in str(e) or 'RateLimitError' in str(e) or 'quota' in str(e).lower():
            st.warning("OpenAI quota exceeded or invalid key. Falling back to HuggingFace embeddings (all-MiniLM-L6-v2). This may be slower but does not require an API key.")
...
st.markdown("<div style='text-align: center;'>Upload a PDF or Excel file, then chat below.</div>", unsafe_allow_html=True)

if st.session_state.vectorstore:
    chat_container = st.container()
    with chat_container:
        for i, (q, a) in enumerate(st.session_state.chat_history[::-1]):
            st.markdown(f"<div style='background:#f1f1f1;padding:8px;border-radius:8px;margin-bottom:4px;'><b>You:</b> {q}</div>", unsafe_allow_html=True)
            st.markdown(f"<div style='background:#e8f5e9;padding:8px;border-radius:8px;margin-bottom:12px;'><b>Bot:</b> {a}</div>", unsafe_allow_html=True)
    ...
    if user_input:
        ...
        try:
            ...
            if 'insufficient_quota' in str(e) or 'RateLimitError' in str(e) or 'quota' in str(e).lower():
                st.warning("OpenAI LLM quota exceeded or invalid key. Falling back to local HuggingFace pipeline (distilbert-base-uncased). This does not require an API key or token, but answers may be less accurate.")
                try:
                    docs = retriever.get_relevant_documents(user_input)
                    if not docs:
                        answer = "Sorry, I couldn't find relevant information in the document."
                    else:
                        # Concatenate up to 3 chunks for context
